...
exports.OptionCombinationHandler = void 0;
class OptionCombinationHandler {
    constructor(options) {
    ...
    convertCombinationsToRows(combinations) {
        ...
        this.optionKeys.map((optionKey) => {
            this.options[optionKey].map((optionItemKey) => {
    ...
    convertRowsToCombinations(rows) {
        let combinations = [];
        rows.map((row) => {
            combinations = combinations.concat(this.convertRowToCombinations(row));
        });
        return combinations;
    }
        ...
        combinations.map((combination) => {
            this.optionKeys.map((optionKey) => {
                ...
                if (!row[optionKey].keys.includes(combination[optionKey])) {
                    ...
                    if (row[optionKey].keys.length === this.options[optionKey].length) {
                ...
                if (Object.keys(diff).length === 1) {
                    this.optionKeys.map((optionKey) => {
                        if (diff[optionKey] && !row[optionKey].keys.includes(diff[optionKey])) {
                            ...
                            if (this.options[optionKey].length === row[optionKey].keys.length) {
                                row[optionKey].isAll = true;
                            }
                            return;
                        }
    ...
    generateCombinations(index, result = {}) {
        ...
        this.options[optionKey].map((key) => {
...
exports.OptionCombinationHandler = OptionCombinationHandler;
//# sourceMappingURL=OptionCombinationHandler.js.map